...
Further evaluations of this study can be reproduced using our expanded version of [bias-bench](todo).
...
Optional: Install [`bias-bench`](todo) for further evaluations and comparison to other debiasing techniques.
...
- `analyze_decoder.evaluate_all_gender_predictions()` and `export/gender_predictions.py` ... overfitting analysis
- `export/example_predictions.py` to generate example predictions

### Evaluation of (De-)Biased Models
See [bias-bench](todo link) for a comparison of the (de-)biased models generated with GRADIEND to other debiasing techniques.
...
`export/example_predictions` | Generates example predictions for the selected (de-) biased as a LaTeX table
